package site.lbw.util.telegram;
...
import site.lbw.config.properties.TelegramProperties;
import site.lbw.model.dto.Comment;
import site.lbw.entity.Moment;
import site.lbw.enums.CommentPageEnum;
import site.lbw.service.CommentService;
import site.lbw.service.MomentService;
import site.lbw.util.comment.CommentUtils;
...
public class TelegramBotMsgHandler {
	...
	private String replyComment(String msgContent) {
		...
		if (splitIndex2 != -1 && msgContent.length() > splitIndex2 + 1) {
			...
			//先找到要回复的评论
			site.lbw.entity.Comment parentComment = commentService.getCommentById(commentId);
